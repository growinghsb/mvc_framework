...
* 추상화 하는 방법을 모르겠다. 경험이 부족한건지, 공부가 부족한건지       
  ...
  리팩토링을 해보자.     
***      
* 각각 데이터를 가지고 있는 AdapterMapping과 HandlerAdapter    
  class 내부로 Controller와 Adapter를 반환하는 로직을   
  집어 넣음으로써 응집도를 높혔다.    
  이렇게 진행한 이유는 클라이언트는 메시지만을 전달하기 위함이다.    
  물론 get 메서드를 통해 데이터를 요청하고 있지만 FrontController   
  특성상 전략패턴으로 구현 되어 있기 때문에 이정도는 감수해야 될 듯하다.    
  
* 하지만 관련있는 데이터는 요청을 함으로써 협력을 하고,   
  그 외 의존성이 있는 클래스들은 초대한 캡슐화 함으로써   
  응집도를 높히고, 추후 확장시에도 클라이언트인    
  FrontController에는 아무런 수정이 필요없다.     
  
* 이제 어노테이션으로 처리 가능한 Controller와 Adapter를 만들어    
  한 번 더 확장을 해봐야겠다.   
[어노테이션 정리](https://github.com/growinghsb/til_blog/blob/main/April/1-2.md)        

* 어노테이션으로 처리 가능한 컨트롤러를 만들려면    
  일단 어노테이션 내부에서 url이 구분 가능해야 한다.     
  그리고 인터페이스를 상속받을 필요가 없다.    
  어노테이션 하나로 전부 처리가 가능하다.   
  메서드단위로 붙어있는 어노테이션의 정보를 가져올 수 있고,    
  해당 메서드를 가져올 수도 있다.   
  
* 결국 컨트롤러 클래스 한 개에 직접적으로 요청을   
  처리할 메서드 3개, 그리고 어댑터만 있으면 된다.    
  
* 어노테이션은 기존것을 이용할 수 있지만 연습을 위해    
  직접 만들어서 사용해보자.
***     
* 직접 어노테이션 만들고 구현까지 했다.   
  하지만 확장성면에서는 꽝이다.   
  컨트롤러를 추가하면 해당 로직을 추가해야 하고,   
  만약 매개변수 개수가 같은데 순서가 다르다면...     
  모르겠다. 이건 아닌거 같은데 일단 구현만 했다.     
  
* 리팩토링 하고 스프링 없이 게시판 구현하는거    
  기획하고 진행해야겠다.   
